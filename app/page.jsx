...
const Badge = ({ severity, label, style = {} }) => {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "1px 6px", borderRadius: 4, fontSize: 11, fontWeight: 600, letterSpacing: "0.02em", textTransform: "uppercase", color: meta.color, background: meta.bg, border: `1px solid ${meta.border}`, ...style }}>
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "1px 6px", borderRadius: 4, fontSize: 11, fontWeight: 600, letterSpacing: "0.02em", textTransform: "uppercase", color: meta.color, background: meta.bg, border: `1px solid ${meta.border}` }}>
      {status === "running" && <span style={{ width: 6, height: 6, borderRadius: "50%", background: meta.color, animation: "pulse 1.5s infinite" }} />}
      {meta.label}
    </span>
  );
};
...
const Btn = ({ children, variant = "primary", onClick, disabled, style = {}, size = "md" }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ display: "inline-flex", alignItems: "center", gap: 6, border: "1px solid", borderRadius: 6, fontWeight: 500, fontFamily: "inherit", cursor: disabled ? "not-allowed" : "pointer", transition: "all 0.12s ease", opacity: disabled ? 0.5 : 1, fontSize: size === "sm" ? 12 : 13, padding: size === "sm" ? "4px 10px" : "6px 14px", ...variants[variant], ...style }}
      onMouseEnter={e => { if (disabled) return; if (variant === "primary") e.currentTarget.style.background = "#4338ca"; else if (variant === "secondary") e.currentTarget.style.background = "#f9fafb"; }}
...
const Input = ({ label, id, placeholder, value, onChange, type = "text", monospace, helper, error }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
    ...
    <input id={id} name={id} type={type} placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)}
      style={{ padding: "7px 10px", borderRadius: 6, fontSize: 13, border: `1px solid ${error ? "#fca5a5" : "#d1d5db"}`, background: error ? "#fff5f5" : "#fff", color: "#111827", outline: "none", fontFamily: monospace ? "'JetBrains Mono', monospace" : "inherit" }}
...
const Spinner = ({ size = 16 }) => (
  <div style={{ width: size, height: size, borderRadius: "50%", border: "2px solid #e5e7eb", borderTopColor: "#4f46e5", animation: "spin 0.6s linear infinite", flexShrink: 0 }} />
);
...
const ErrorBanner = ({ message, onDismiss }) => (
  <div style={{ padding: "10px 14px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 6, fontSize: 12, color: "#dc2626", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
    <span>✗ {message}</span>
...
const ConnectedBadge = ({ connected }) => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 4, color: connected ? "#059669" : "#6b7280", background: connected ? "#ecfdf5" : "#f3f4f6", border: `1px solid ${connected ? "#a7f3d0" : "#e5e7eb"}`, textTransform: "uppercase", letterSpacing: "0.04em" }}>
...
const DiffView = ({ expected, actual }) => {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    } else { lines.push({ type: "same", text: l }); }
  }
  return (
      ...
      <div style={{ padding: "8px 0", maxHeight: 200, overflowY: "auto" }}>
        {lines.map((line, i) => (
          <div key={i} style={{ display: "flex", padding: "1px 12px", background: line.type === "remove" ? "rgba(248,81,73,0.12)" : line.type === "add" ? "rgba(63,185,80,0.12)" : "transparent" }}>
            ...
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11.5, color: line.type === "remove" ? "#ffa198" : line.type === "add" ? "#7ee787" : "#e6edf3", whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
              {line.text}
            </span>
          </div>
        ))}
            ...
            <div style={{ padding: "32px 28px" }}>
              <div style={{ textAlign: "center", marginBottom: 28 }}>
                ...
                <div style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.6 }}>
                  We sent a verification code to<br /><strong style={{ color: "#374151" }}>{email}</strong>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  placeholder="00000000" value={otp} maxLength={8}
                  onChange={e => setOtp(e.target.value.replace(/\D/g, ""))}
                  autoFocus
                  style={{ width: "100%", padding: "14px 16px", borderRadius: 8, fontSize: 36, fontWeight: 700, letterSpacing: 14, textAlign: "center", border: "1px solid #d1d5db", background: "#fff", color: "#4f46e5", outline: "none", fontFamily: "'JetBrains Mono', monospace", boxSizing: "border-box" }}
                  onFocus={e => { e.target.style.borderColor = "#4f46e5"; e.target.style.boxShadow = "0 0 0 3px rgba(79,70,229,0.08)"; }}
                  onBlur={e => { e.target.style.borderColor = "#d1d5db"; e.target.style.boxShadow = "none"; }}
                />
              </div>
              <Btn variant="primary" onClick={verifyOtp} disabled={loading || otp.length < 6} style={{ width: "100%", justifyContent: "center", padding: "10px 0" }}>
                {loading ? <><Spinner size={13} />Verifying...</> : "Verify & continue →"}
              </Btn>
              <div style={{ textAlign: "center", marginTop: 16 }}>
                <button onClick={() => { setOtpSent(false); setOtp(""); setError(null); }} style={{ fontSize: 12, color: "#9ca3af", background: "none", border: "none", cursor: "pointer" }}>← Back</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, overflow: "hidden" }}>
          ...
          <div style={{ padding: "28px 28px 20px" }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#111827", marginBottom: 4 }}>{mode === "signin" ? "Sign in to your account" : "Create your account"}</div>
            <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 24 }}>{mode === "signin" ? "Welcome back" : "Start your 14-day free trial — no card required"}</div>
            {error && <ErrorBanner message={error} onDismiss={() => setError(null)} />}
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {mode === "signup" && <Input id="company" label="Company name" placeholder="Acme Corp" value={company} onChange={setCompany} />}
              ...
              <Btn variant="primary" onClick={submit} disabled={loading || !email || !password || (mode === "signup" && !company)} style={{ width: "100%", justifyContent: "center", padding: "9px 0" }}>
          ...
          <div style={{ padding: "14px 28px", background: "#f9fafb", borderTop: "1px solid #f3f4f6", textAlign: "center" }}>
            <span style={{ fontSize: 12, color: "#6b7280" }}>{mode === "signin" ? "Don't have an account? " : "Already have an account? "}</span>
            <button onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setError(null); }} style={{ fontSize: 12, color: "#4f46e5", fontWeight: 600, background: "none", border: "none", cursor: "pointer" }}>
...
const Sidebar = ({ page, setPage, scanning, onScan, lastScan, openDriftCount, org, onSignOut }) => (
  <aside style={{ width: 240, flexShrink: 0, height: "100vh", position: "sticky", top: 0, background: "#fff", borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", overflow: "hidden" }}>
    <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid #f3f4f6" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
        <div style={{ width: 28, height: 28, borderRadius: 6, background: "#4f46e5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#fff", fontWeight: 700, flexShrink: 0 }}>D</div>
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#111827", letterSpacing: "-0.01em" }}>DriftWatch</div>
          <div style={{ fontSize: 10, color: "#9ca3af" }}>{org?.name || "Loading..."} · {org?.plan || "—"}</div>
        </div>
      </div>
    </div>
      ...
      {NAV_ITEMS.map(item => {
        ...
        return (
          <button key={item.id} onClick={() => setPage(item.id)}
            style={{ width: "100%", display: "flex", alignItems: "center", gap: 9, padding: "7px 10px", borderRadius: 5, border: "none", cursor: "pointer", background: active ? "#f5f3ff" : "transparent", color: active ? "#4f46e5" : "#6b7280", fontWeight: active ? 600 : 400, fontSize: 13, textAlign: "left", marginBottom: 1, transition: "all 0.1s" }}
          ...
          >
            ...
            {item.id === "drift" && openDriftCount > 0 && (
              <span style={{ marginLeft: "auto", background: "#dc2626", color: "#fff", borderRadius: 10, fontSize: 10, padding: "0 5px", fontWeight: 700, minWidth: 18, textAlign: "center" }}>{openDriftCount}</span>
            )}
          </button>
        );
      })}
    ...
    <div style={{ padding: "12px", borderTop: "1px solid #f3f4f6" }}>
      <button onClick={onScan} disabled={scanning}
        style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid", fontWeight: 600, fontSize: 13, cursor: scanning ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, background: scanning ? "#f5f3ff" : "#4f46e5", borderColor: scanning ? "#c4b5fd" : "#4338ca", color: scanning ? "#4f46e5" : "#fff", transition: "all 0.2s", animation: scanning ? "scanPulse 1.5s infinite" : "none" }}>
      ...
      <button onClick={onSignOut} style={{ width: "100%", marginTop: 8, padding: "6px", background: "none", border: "none", fontSize: 11, color: "#9ca3af", cursor: "pointer", textAlign: "center" }}
...
const StatCard = ({ label, value, sub, accent, loading }) => (
  <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: "14px 16px", flex: 1, minWidth: 120 }}>
    ...
    <div style={{ fontSize: 24, fontWeight: 700, color: loading ? "#e5e7eb" : (accent || "#111827"), letterSpacing: "-0.02em", lineHeight: 1 }}>{loading ? "—" : value}</div>
...
const DriftTable = ({ items, loading, compact, scanMap }) => {
  ...
  if (!items || items.length === 0) return <EmptyState icon="✓" title="No drift detected" body="All resources match their Terraform state." />;
  return (
        ...
        {items.map((item, i) => (
          <>
            <tr key={item.id} onClick={() => setExpanded(expanded === item.id ? null : item.id)}
              ...
              <td style={{ padding: "8px 12px" }}><code style={{ fontSize: 11.5, color: "#374151", background: "#f3f4f6", padding: "1px 5px", borderRadius: 3 }}>{item.resource_type}</code></td>
              <td style={{ padding: "8px 12px" }}><code style={{ fontSize: 11.5, color: "#4f46e5" }}>{item.resource_id}</code></td>
              ...
              {!compact && <td style={{ padding: "8px 12px", fontSize: 11, color: "#9ca3af", whiteSpace: "nowrap" }}>{scanMap?.[item.scan_id] ? fmt.relative(scanMap[item.scan_id].started_at) : "—"}</td>}
              <td style={{ padding: "8px 12px", color: "#9ca3af", fontSize: 12 }}>{expanded === item.id ? "▾" : "▸"}</td>
            </tr>
            {expanded === item.id && (
              <tr key={`${item.id}-exp`} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td colSpan={compact ? 6 : 7} style={{ padding: "12px 16px", background: "#fafafa" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "8px 16px", marginBottom: 10 }}>
                    <span style={{ fontSize: 11, color: "#9ca3af", fontWeight: 500 }}>Resource ID</span><code style={{ fontSize: 11.5, color: "#4f46e5" }}>{item.resource_id}</code>
                    <span style={{ fontSize: 11, color: "#9ca3af", fontWeight: 500 }}>Type</span><code style={{ fontSize: 11.5 }}>{item.resource_type}</code>
                    <span style={{ fontSize: 11, color: "#9ca3af", fontWeight: 500 }}>Scan ID</span><code style={{ fontSize: 11.5, color: "#6b7280" }}>{item.scan_id}</code>
                  </div>
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      ...
      <tbody>
        {scans.map((scan, i) => (
          <>
            <tr key={scan.id} onClick={() => toggle(scan.id)}
              ...
              <tr key={`${scan.id}-exp`} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td colSpan={7} style={{ padding: "12px 16px", background: "#fafafa" }}>
                  {scan.error_message ? (
                    ...
                    <><div style={{ fontSize: 12, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Drift items from this scan</div><DriftTable items={scanDrift[scan.id]} compact /></>
                  ) : (
...
const DashboardPage = ({ orgId, scanning, setPage }) => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [{ data: scansData, error: se }, { data: driftData, error: de }] = await Promise.all([
  ...
  const successRate = scans.length > 0 ? Math.round((scans.filter(s => s.status === "completed").length / scans.length) * 100) : null;

  return (
    <div className="fadeIn">
      <PageHeader title="Dashboard" subtitle={latestScan ? `Last scan ${fmt.relative(latestScan.started_at)} · ${latestScan.resources_checked} resources` : "No scans yet — connect AWS and run your first scan"} right={latestScan && <StatusBadge status={latestScan.status} />} />
      {error && <ErrorBanner message={error} onDismiss={() => setError(null)} />}
...
const ScansPage = ({ orgId }) => {
  ...
  useEffect(() => {
    ...
    supabase.from("scans").select("*").eq("org_id", orgId).gte("started_at", since.toISOString()).order("started_at", { ascending: false })
      .then(({ data, error }) => { if (error) setError(error.message); else setScans(data || []); setLoading(false); });
  }, [orgId, range]);

  return (
    <div className="fadeIn">
      <PageHeader title="Scan History" subtitle="All infrastructure scans — expand rows to view drift details"
        right={<SelectInput id="range" value={range} onChange={setRange} options={[{ value: "7", label: "Last 7 days" }, { value: "30", label: "Last 30 days" }, { value: "90", label: "Last 90 days" }]} />} />
      {error && <ErrorBanner message={error} onDismiss={() => setError(null)} />}
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden" }}>
...
const DriftPage = ({ orgId }) => {
  ...
  return (
    <div className="fadeIn">
      <PageHeader title="Drift Items" subtitle={`${drift.length} open items — expand any row to view config diff`}
        right={<Btn variant="secondary" size="sm" onClick={() => {
          ...
          const a = document.createElement("a"); a.href = "data:text/csv;charset=utf-8," + encodeURIComponent(csv); a.download = "drift-items.csv"; a.click();
        }}>↓ Export CSV</Btn>} />
      {error && <ErrorBanner message={error} onDismiss={() => setError(null)} />}
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {["all", "critical", "warning", "info"].map(sev => {
          ...
          const meta = severityMeta[sev];
          return (
            <button key={sev} onClick={() => setSevFilter(sev)} style={{ padding: "4px 12px", borderRadius: 5, border: "1px solid", fontSize: 12, fontWeight: 500, cursor: "pointer", background: active ? (meta?.bg || "#f5f3ff") : "#fff", color: active ? (meta?.color || "#4f46e5") : "#6b7280", borderColor: active ? (meta?.border || "#c4b5fd") : "#e5e7eb" }}>
              {sev === "all" ? "All" : sev.charAt(0).toUpperCase() + sev.slice(1)}
            </button>
          );
        })}
        <SelectInput id="typeFilter" value={typeFilter} onChange={setTypeFilter} options={[{ value: "all", label: "All types" }, { value: "MISSING", label: "Missing" }, { value: "UNMANAGED", label: "Unmanaged" }, { value: "DRIFTED", label: "Drifted" }]} />
        {resourceTypes.length > 0 && <SelectInput id="resType" value="all" onChange={() => {}} options={[{ value: "all", label: "All resource types" }, ...resourceTypes.map(t => ({ value: t, label: t }))]} />}
      </div>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden" }}>
...
const SettingsCard = ({ title, desc, children, action }) => (
  <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, marginBottom: 16, overflow: "hidden" }}>
    ...
    {action && <div style={{ padding: "12px 16px", background: "#f9fafb", borderTop: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>{action}</div>}
  </div>
);
...
const SettingsPage = ({ orgId, org }) => {
  ...
  useEffect(() => {
    ...
    ]).then(([aws, state, slack]) => { setAwsConn(aws.data); setStateBackend(state.data); setSlackConn(slack.data); });
  }, [orgId]);

  const tabs = [{ id: "aws", label: "AWS" }, { id: "state", label: "State Backend" }, { id: "slack", label: "Slack" }, { id: "schedule", label: "Schedule" }, { id: "billing", label: "Billing" }];
  ...
  return (
    <div className="fadeIn">
      ...
      <div style={{ display: "flex", marginBottom: 20, borderBottom: "1px solid #e5e7eb" }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: "8px 16px", border: "none", background: "transparent", cursor: "pointer", fontSize: 13, fontWeight: 500, color: activeTab === tab.id ? "#4f46e5" : "#6b7280", borderBottom: `2px solid ${activeTab === tab.id ? "#4f46e5" : "transparent"}`, marginBottom: -1, transition: "all 0.1s" }}>{tab.label}</button>
        ))}
      </div>
      ...
      {activeTab === "slack" && <SettingsSlack orgId={orgId} conn={slackConn} onSave={c => { setSlackConn(c); showToast("Slack connected"); }} onError={e => showToast(e, "error")} />}
...
const SettingsAWS = ({ orgId, conn, onSave, onError }) => {
  ...
  useEffect(() => { if (conn) { setAuthMode(conn.auth_mode || "role"); setRoleArn(conn.role_arn || ""); setRegion(conn.region || "us-east-1"); } }, [conn]);

  const verify = async () => {
    setVerifying(true);
    try {
      const res = await fetch("/api/connections/aws", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ org_id: orgId, auth_mode: authMode, role_arn: roleArn, region, access_key: accessKey, secret_key: secretKey }) });
  ...
  return (
    <SettingsCard title="AWS Authentication" desc="Read-only access to compare live infrastructure against Terraform state."
      action={<><ConnectedBadge connected={!!conn?.verified_at} /><Btn variant="primary" onClick={verify} disabled={verifying}>{verifying ? <><Spinner size={12} />Verifying...</> : conn?.verified_at ? "Re-verify" : "Verify & save"}</Btn></>}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {["role", "keys"].map(m => (
          <button key={m} onClick={() => setAuthMode(m)} style={{ padding: "5px 14px", borderRadius: 5, border: "1px solid", fontSize: 12, fontWeight: 500, cursor: "pointer", background: authMode === m ? "#f5f3ff" : "#fff", color: authMode === m ? "#4f46e5" : "#6b7280", borderColor: authMode === m ? "#c4b5fd" : "#e5e7eb" }}>
      ...
      {authMode === "role" ? (
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ padding: "10px 12px", background: "#f8f9fa", border: "1px solid #e5e7eb", borderRadius: 6 }}>
            ...
            <code style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.8, display: "block" }}>ec2:Describe* · rds:Describe* · s3:GetObject · s3:GetBucketVersioning<br />lambda:ListFunctions · ecs:Describe* · ecs:List* · iam:GetRole · sts:GetCallerIdentity</code>
          </div>
        </div>
      ) : (
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ padding: "8px 10px", background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 5, fontSize: 11, color: "#92400e" }}>⚠ Encrypted with AES-256-GCM. IAM roles recommended for production.</div>
        </div>
      )}
      <div style={{ marginTop: 12 }}>
        <SelectInput id="awsRegion" label="AWS Region" value={region} onChange={setRegion} options={[
          { value: "us-east-1", label: "us-east-1 (N. Virginia)" }, { value: "us-west-2", label: "us-west-2 (Oregon)" },
          { value: "eu-west-1", label: "eu-west-1 (Ireland)" }, { value: "eu-central-1", label: "eu-central-1 (Frankfurt)" },
          { value: "ap-southeast-1", label: "ap-southeast-1 (Singapore)" }, { value: "ap-northeast-1", label: "ap-northeast-1 (Tokyo)" },
        ]} />
      </div>
    </SettingsCard>
  );
...
const SettingsState = ({ orgId, backend, onSave, onError }) => {
  ...
  useEffect(() => { if (backend) { setBucket(backend.bucket || ""); setKey(backend.key || ""); setRegion(backend.region || "us-east-1"); } }, [backend]);

  const verify = async () => {
    setVerifying(true);
    try {
      const res = await fetch("/api/connections/state", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ org_id: orgId, bucket, key, region }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Verification failed");
  ...
  return (
    <SettingsCard title="Terraform State Backend" desc="S3 bucket where your Terraform state file is stored."
      action={<><ConnectedBadge connected={!!backend?.verified_at} /><Btn variant="primary" onClick={verify} disabled={verifying || !bucket || !key}>{verifying ? <><Spinner size={12} />Verifying...</> : backend?.verified_at ? "Re-verify" : "Verify & save"}</Btn></>}>
      <div style={{ display: "grid", gap: 12 }}>
        <Input id="bucket" label="S3 Bucket Name" value={bucket} onChange={setBucket} monospace placeholder="my-terraform-state-bucket" />
        <Input id="stateKey" label="State File Key" value={key} onChange={setKey} monospace placeholder="prod/terraform.tfstate" helper="Path to the .tfstate file within the bucket" />
        <SelectInput id="stateRegion" label="Bucket Region" value={region} onChange={setRegion} options={[
          ...
          { value: "eu-west-1", label: "eu-west-1" }, { value: "eu-central-1", label: "eu-central-1" },
...
const SettingsSlack = ({ orgId, conn, onSave, onError }) => {
  ...
  const verify = async () => {
    setVerifying(true);
    try {
      const res = await fetch("/api/connections/slack", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ org_id: orgId, webhook_url: webhook, channel }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Verification failed");
  ...
  return (
    <SettingsCard title="Slack Notifications" desc="Receive drift alerts in your Slack workspace. A test message is sent on save."
      action={<><ConnectedBadge connected={!!conn?.verified_at} /><Btn variant="primary" onClick={verify} disabled={verifying || !webhook}>{verifying ? <><Spinner size={12} />Connecting...</> : conn?.verified_at ? "Update & test" : "Connect & test"}</Btn></>}>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        <Input id="slackChannel" label="Channel" value={channel} onChange={setChannel} placeholder="#infra-alerts" helper="Optional" />
      </div>
    </SettingsCard>
  );
};
...
const SettingsSchedule = ({ orgId, org, onSave, onError }) => {
  ...
  return (
    <SettingsCard title="Scan Schedule" desc="How often DriftWatch checks your infrastructure against Terraform state."
      action={<><span style={{ fontSize: 12, color: "#9ca3af" }}>Runs daily at 06:00 UTC</span><Btn variant="primary" onClick={save} disabled={saving}>{saving ? <><Spinner size={12} />Saving...</> : "Save schedule"}</Btn></>}>
      <div style={{ display: "grid", gap: 8 }}>
        {[{ value: "1h", label: "Every hour", plan: "Pro" }, { value: "6h", label: "Every 6 hours", plan: "Pro" }, { value: "24h", label: "Daily", plan: "Starter" }, { value: "manual", label: "Manual only", plan: "Starter" }].map(f => (
          <label key={f.value} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 6, border: "1px solid", borderColor: freq === f.value ? "#c4b5fd" : "#e5e7eb", background: freq === f.value ? "#f5f3ff" : "#fff", cursor: "pointer" }}>
        ...
        {plans.map(plan => {
          ...
          return (
            <div key={plan.name} style={{ background: "#fff", border: `1px solid ${current ? "#4f46e5" : "#e5e7eb"}`, borderRadius: 8, padding: "16px", position: "relative", boxShadow: current ? "0 0 0 1px #4f46e5" : "none" }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: "#111827", marginBottom: 12 }}>${plan.price}<span style={{ fontSize: 13, fontWeight: 400, color: "#9ca3af" }}>/mo</span></div>
              <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 16 }}>
                {plan.features.map(f => <div key={f} style={{ fontSize: 12, color: "#6b7280", display: "flex", gap: 6 }}><span style={{ color: "#059669", flexShrink: 0 }}>✓</span> {f}</div>)}
              </div>
              <Btn variant={current ? "secondary" : "primary"} style={{ width: "100%", justifyContent: "center" }}>{current ? "Manage subscription" : "Upgrade"}</Btn>
            </div>
          );
        })}
      </div>
      {org && (
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: "14px 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            <div><div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 3 }}>Plan</div><div style={{ fontSize: 13, fontWeight: 500, color: "#111827", textTransform: "capitalize" }}>{org.plan} · ${org.plan === "starter" ? 29 : org.plan === "pro" ? 99 : 299}/mo</div></div>
            <div><div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 3 }}>Status</div><StatusBadge status={org.plan_status === "active" ? "completed" : org.plan_status === "trialing" ? "running" : "failed"} /></div>
            <div><div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 3 }}>Member since</div><div style={{ fontSize: 13, fontWeight: 500, color: "#111827" }}>{fmt.ts(org.created_at)}</div></div>
          </div>
        </div>
      )}
    </>
...
const OnboardingPage = ({ onComplete, orgId: propOrgId }) => {
  ...
  useEffect(() => {
    ...
    const fetchOrg = async () => {
      for (let i = 0; i < 10; i++) {
        ...
        if (data?.id) { setOrgId(data.id); return; }
        await new Promise(r => setTimeout(r, 1000));
      }
      setError("Could not load organization. Please refresh the page.");
    };
    fetchOrg();
  }, [orgId]);
  ...
  const verifyStep = async (idx) => {
    if (!orgId) { setError("Organization not loaded yet. Please wait a moment and try again."); return; }
    setError(null);
    setVerifying(true);
    try {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 560 }}>
        ...
        {!orgId && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, marginBottom: 16, fontSize: 13, color: "#6b7280" }}>
            <Spinner size={14} /> Setting up your organization...
          </div>
        )}
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, overflow: "hidden" }}>
          {step === 0 && (
            <OnboardStep title="Connect AWS Account" desc="Grant DriftWatch read-only access to your AWS account via an IAM role." onVerify={() => verifyStep(0)} verified={verified[0]} verifying={verifying} onNext={() => setStep(1)}>
              <Input id="onboardRoleArn" label="IAM Role ARN" monospace placeholder="arn:aws:iam::123456789012:role/DriftWatchRole" value={stepData.roleArn} onChange={v => setStepData(d => ({ ...d, roleArn: v }))} helper="Paste the ARN of the role you created in IAM" />
              <SelectInput id="onboardRegion" label="Primary Region" value={stepData.region} onChange={v => setStepData(d => ({ ...d, region: v }))} options={[
                { value: "us-east-1", label: "us-east-1 (N. Virginia)" }, { value: "us-west-2", label: "us-west-2 (Oregon)" },
                { value: "eu-west-1", label: "eu-west-1 (Ireland)" }, { value: "ap-southeast-1", label: "ap-southeast-1 (Singapore)" },
              ]} />
            </OnboardStep>
          )}
          {step === 1 && (
          ...
          {step === 3 && (
            <div style={{ padding: "28px 24px", textAlign: "center" }}>
              ...
              {!scanResult && !scanning && (
                <div style={{ marginTop: 12 }}>
                  <button onClick={onComplete} style={{ fontSize: 12, color: "#9ca3af", background: "none", border: "none", cursor: "pointer" }}>Skip for now →</button>
                </div>
              )}
            </div>
          )}
  ...
  <div style={{ minHeight: "100vh", background: "#fff" }}>
    <nav style={{ borderBottom: "1px solid #e5e7eb", padding: "0 48px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "rgba(255,255,255,0.96)", backdropFilter: "blur(8px)", zIndex: 100 }}>
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
        {["Product", "Pricing", "Docs"].map(l => <span key={l} style={{ fontSize: 13, color: "#6b7280", cursor: "pointer", fontWeight: 500 }}>{l}</span>)}
    ...
    <section style={{ maxWidth: 920, margin: "0 auto", padding: "80px 48px 64px", textAlign: "center" }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 5, background: "#f5f3ff", border: "1px solid #c4b5fd", fontSize: 11, fontWeight: 600, color: "#4f46e5", marginBottom: 24, letterSpacing: "0.04em" }}>● LIVE · Infrastructure drift detection in real-time</div>
      <h1 style={{ fontSize: 48, fontWeight: 800, color: "#111827", lineHeight: 1.1, letterSpacing: "-0.03em", marginBottom: 20 }}>Terraform drift,<br />caught before it costs you.</h1>
      <p style={{ fontSize: 17, color: "#6b7280", lineHeight: 1.6, maxWidth: 560, margin: "0 auto 36px" }}>DriftWatch continuously compares your live AWS infrastructure against Terraform state. Catch unauthorized changes, missing resources, and configuration drift — before your 2am incident.</p>
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
        <Btn variant="primary" onClick={onGetStarted} style={{ padding: "10px 24px", fontSize: 14 }}>Start free trial →</Btn>
        <Btn variant="secondary" style={{ padding: "10px 24px", fontSize: 14 }}><span style={{ fontFamily: "monospace", fontSize: 12 }}>POST /api/scans/run</span></Btn>
      </div>
    ...
    <section style={{ background: "#f9fafb", borderTop: "1px solid #e5e7eb", borderBottom: "1px solid #e5e7eb", padding: "64px 48px" }}>
      <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
          {[{ n: "01", title: "Sign up", body: "Create your account. Your organization is provisioned automatically." }, { n: "02", title: "Connect AWS", body: "Provide a read-only IAM role. DriftWatch never modifies your infrastructure." }, { n: "03", title: "Point to state", body: "Tell us your S3 bucket and Terraform state key. We parse it directly." }, { n: "04", title: "Get alerted", body: "Severity-labeled drift delivered to Slack within seconds of detection." }].map(s => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {[
          ...
          <div key={plan.name} style={{ border: `1px solid ${plan.highlight ? "#4f46e5" : "#e5e7eb"}`, borderRadius: 10, padding: "24px", background: "#fff", boxShadow: plan.highlight ? "0 0 0 1px #4f46e5, 0 4px 16px rgba(79,70,229,0.1)" : "none" }}>
            ...
            <div style={{ fontSize: 30, fontWeight: 800, color: "#111827", letterSpacing: "-0.02em", marginBottom: 20 }}>${plan.price}<span style={{ fontSize: 14, fontWeight: 400, color: "#9ca3af" }}>/mo</span></div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
              {plan.features.map(f => <div key={f} style={{ display: "flex", gap: 8, fontSize: 12, color: "#374151" }}><span style={{ color: "#059669", flexShrink: 0 }}>✓</span> {f}</div>)}
            </div>
    ...
    <footer style={{ borderTop: "1px solid #e5e7eb", padding: "24px 48px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", gap: 20 }}>
        {["Privacy", "Terms", "Status", "Docs"].map(l => <span key={l} style={{ fontSize: 12, color: "#9ca3af", cursor: "pointer" }}>{l}</span>)}
      </div>
    </footer>
  </div>
);
...
export default function DriftWatch() {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          if (data) { orgData = data; break; }
          await new Promise(r => setTimeout(r, 800));
        }
        if (orgData) {
          setOrg(orgData);
          setOrgId(orgData.id);
          const { data: conn } = await supabase.from("aws_connections").select("id").eq("org_id", orgData.id).maybeSingle();
          setView(conn ? "app" : "onboarding");
        } else {
          setView("onboarding");
        }
      } else {
  ...
  useEffect(() => {
    ...
    supabase.from("scans").select("*").eq("org_id", orgId).order("started_at", { ascending: false }).limit(1).then(({ data }) => { if (data?.[0]) setLastScan(data[0]); });
    supabase.from("drift_items").select("id", { count: "exact", head: true }).eq("org_id", orgId).is("resolved_at", null).then(({ count }) => { setOpenDriftCount(count || 0); });
  }, [view, orgId, scanning]);
  ...
  if (view === "landing") return <><style>{FONTS}{css}</style><LandingPage onGetStarted={() => setView("auth")} onSignIn={() => setView("auth")} /></>;
  if (view === "auth") return <><style>{FONTS}{css}</style><AuthPage onAuth={async (session, isNew) => {
    let orgData = null;
    for (let i = 0; i < 8; i++) {
      const { data } = await supabase.from("organizations").select("*").limit(1).maybeSingle();
      if (data) { orgData = data; break; }
    ...
    if (orgData) {
      ...
      setOrgId(orgData.id);
      const { data: conn } = await supabase.from("aws_connections").select("id").eq("org_id", orgData.id).maybeSingle();
      setView(isNew || !conn ? "onboarding" : "app");
    } else {
      setView("onboarding");
    }
  }} /></>;
  if (view === "onboarding") return <><style>{FONTS}{css}</style><OnboardingPage orgId={orgId} onComplete={() => setView("app")} /></>;
